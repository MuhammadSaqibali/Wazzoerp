        ...
        if(Auth::check())
        {
            if(\Auth::user()->can('show hrm dashboard'))
            {
                ...
                else
                {
                    ...
                    return view('dashboard.dashboard', compact('employeeAttendance','arrEvents', 'onGoingTraining', 'activeJob', 'inActiveJOb', 'doneTraining', 'announcements', 'employees', 'meetings', 'countTrainer', 'countClient', 'countUser', 'notClockIns', 'countEmployee'));
                }
            }
            else
            {
