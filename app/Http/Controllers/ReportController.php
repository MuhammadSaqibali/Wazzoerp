...
// --------------------------------------------Custom Expense------------------------------------------------------------------------------------
                ...
                $cexpenses = add__Expense::selectRaw('sum(add_expense.totalexp) as amount,MONTH(date) as month,YEAR(date) as year,category_id')->leftjoin('product_service_categories', 'payments.category_id', '=', 'product_service_categories.id')->where('product_service_categories.type', '=', 2);
                $cexpenses->whereRAW('YEAR(date) =?', [$year]);
                $cexpenses->groupBy('month', 'year', 'category_id');
                $ctmpArray = [];
                foreach ($cexpenses as $expense) {
                    $ctmpArray[$expense->category_id][$expense->month] = $expense->amount;
                }
                $carray = [];
                foreach ($ctmpArray as $cat_id => $record) {
                    $tmp = [];
                    $tmp['category'] = !empty(ProductServiceCategory::where('id', '=', $cat_id)->first()) ? ProductServiceCategory::where('id', '=', $cat_id)->first()->name : '';
                    $tmp['data'] = [];
                    for ($i = 1; $i <= 12; $i++) {
                ...
                try {
                    $cexpensesData = add__Expense::selectRaw('sum(add_expense.totalexp) as amount, MONTH(date) as month, YEAR(date) as year')
                ...
                $cexpenseArr = [];
                foreach ($cexpensesData as $k => $expenseData) {
                    $cexpenseArr[$expenseData->month] = $expenseData->amount;
                }
                for ($i = 1; $i <= 12; $i++) {
                    $cexpenseTotal[] = array_key_exists($i, $cexpenseArr) ? $cexpenseArr[$i] : 0;
                }
                $chartcExpenseArr = array_map(
                    function () {
                        return array_sum(func_get_args());
                    }, $cexpenseTotal
                );
            $data['chartcExpenseArr'] = $chartcExpenseArr;
            $data['chartExpenseArr'] = $chartExpenseArr;
            $data['cexpenseArr'] = $carray;
    ...
    public function dailyAttendance(Request $request)
    {
        if (\Auth::user()->can('manage report')) {
            ...
            $employees = Employee::select('id', 'name')->where('created_by', \Auth::user()->creatorId())->get();

            return view('report.dailyAttendance', compact('employees', 'date'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    ...
    {
        AttendanceEmployee::query()->where('id', $request->attendance_id)->update([
            'overtime' => $request->overtime
        ]);
        return redirect()->route('report.daily.attendance')->with('success', 'Overtime updated');
    }
