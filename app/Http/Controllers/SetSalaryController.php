...
class SetSalaryController extends Controller
{
    ...
    public function edit($id)
    {
        if(\Auth::user()->can('edit set salary'))
        {
            ...
            $totaltaxDeduction = Employee::deductionWithoutIncomeTax($id);
            ...
            if(\Auth::user()->type == 'employee')
            {
                ...
                return view('setsalary.employee_salary', compact('totaltaxDeduction','employee', 'payslip_type', 'allowance_options', 'commissions', 'loan_options', 'overtimes', 'otherpayments', 'saturationdeductions', 'loans', 'deduction_options', 'allowances'));
            ...
            else
            {
                ...
                return view('setsalary.edit', compact('totaltaxDeduction','employee', 'payslip_type', 'allowance_options', 'commissions', 'loan_options', 'overtimes', 'otherpayments', 'saturationdeductions', 'loans', 'deduction_options', 'allowances'));
            }
        }
        else
        {
    ...
    public function show($id)
    {
        ...
        if(\Auth::user()->type == 'employee')
        {
            ...
            $commissions          = Commission::where('employee_id', $currentEmployee->id)->get();
            $loans                = Loan::where('employee_id', $currentEmployee->id)->get();
            $budgets                = Budget::where('employee_id', $currentEmployee->id)->get();
            $iTaxes                = ITax::where('employee_id', $currentEmployee->id)->get();
            $saturationdeductions = SaturationDeduction::where('employee_id', $currentEmployee->id)->get();
            $otherpayments        = OtherPayment::where('employee_id', $currentEmployee->id)->get();
            $overtimes            = Overtime::where('employee_id', $currentEmployee->id)->get();
            $employee             = Employee::where('user_id', '=', \Auth::user()->id)->first();

            return view('setsalary.employee_salary', compact('iTaxes','employee', 'payslip_type', 'allowance_options', 'commissions', 'loan_options', 'overtimes', 'otherpayments', 'saturationdeductions', 'loans', 'deduction_options', 'allowances','budgets'));
        ...
        else
        {
            ...
            $commissions          = Commission::where('employee_id', $id)->get();
            $loans                = Loan::where('employee_id', $id)->get();
            $budgets                = Budget::where('employee_id', $id)->get();
            $iTaxes                = ITax::where('employee_id', $id)->get();
            $saturationdeductions = SaturationDeduction::where('employee_id', $id)->get();
            $otherpayments        = OtherPayment::where('employee_id', $id)->get();
            $overtimes            = Overtime::where('employee_id', $id)->get();
            $employee             = Employee::find($id);

            return view('setsalary.employee_salary', compact('totaltaxDeduction','iTaxes','employee', 'payslip_type', 'allowance_options', 'commissions', 'loan_options', 'overtimes', 'otherpayments', 'saturationdeductions', 'loans', 'deduction_options', 'allowances','budgets'));
        }
    ...
    public function employeeUpdateSalary(Request $request, $id)
    {
        $validator = \Validator::make(
            $request->all(), [
                               ...
                               'salary' => 'required',
                               'salary_payment_method' => 'required',
                               'salary_based' => 'required',
                           ]
        );
        if($validator->fails())
        {
