...
{
    public function bankReconcilation(Request $request)
    {
        ...
        if (\Auth::user()->can('manage transaction')) {
            ...
            return view('bankReconcilation.index', compact('transactions', 'account', 'category', 'filter', 'accounts'));
        } else {
    ...
    public function uploadPdf(Request $request)
    {
        ...
        $stu->pdfDocument = $filename;
        $stu->save();
        if ($stu->save()) {
            // return redirect()->route('pdf.upload.form')->with('success', 'PDF uploaded successfully.');
