                        ...
                        <table class="table table-striped mb-0" id="report-dataTable">
                            ...
                            @endphp
                                ...
                                <tr>
                                    ...
                                    <td>{{ \Carbon\Carbon::parse($payslip->updated_at)->format('d/m/Y')}}</td>
                                    <td>Monthly Salary {{\Carbon\Carbon::parse($payslip->salary_month."-30")->format('M Y')}}</td>
                                    <td>GBP</td>
                                    <td>{{\Auth::user()->priceFormat($payslip->net_payble)}}</td>
                                    <td>---</td>
                                    <td>{{$payslip->employee_id}}</td>
                                    
                                    <td>{{!empty($budget)?$budget->funding:""}}</td>
                                    
                                    <td>{{(!empty($payslip->employees)) ? $payslip->employees->name:''}}</td>
                                    
                                    
                                    <td>{{!empty($budget)?$budget->activity:""}}</td>
                                    <td>{{!empty($budget)?$budget->grant_:""}}</td>
                                    <td>{{!empty($budget)?$budget->functions:""}}</td>
                                    <td>{{!empty($budget)?$budget->country:""}}</td>
                                    <td>{{!empty($budget)?$budget->csopo:""}}</td>
                                    <td>{{!empty($budget)?$budget->donor_report:""}}</td>
